...
readonly ARCH_INSTALLER="$SCRIPT_DIR/install_pkgs.sh"
readonly DOTS_DIR="$REPO_ROOT/dots"
readonly PYTHON_REQUIREMENTS="$REPO_ROOT/sdata/uv/requirements.txt"
...
Bootstrap and install Seam.

On NixOS, this dispatches to setup/nix/install.sh.
On Arch Linux, this installs packages and deploys dotfiles.
...
[[ -r "$REPO_ROOT/logo.txt" ]] || die "Installer logo is missing: $REPO_ROOT/logo.txt"
