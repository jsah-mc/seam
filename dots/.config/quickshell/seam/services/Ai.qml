...
Singleton {
    ...
    Process {
        ...
        function conversationPrompt() {
            const conversation = root.messageIDs
                .map(id => root.messageByID[id])
                .filter(message => message.role !== root.interfaceRole && message.visibleToUser)
                .map(message => `${message.role === "assistant" ? "Assistant" : "User"}: ${message.rawContent}`)
                .join("\n\n")
            return `${root.systemPrompt}\n\nYou are responding inside Seam's desktop chat sidebar. Do not inspect files, run shell commands, or modify the system. Answer the latest user message directly.\n\nConversation:\n${conversation}\n\nAssistant:`
        }
        ...
        stderr: SplitParser {
            onRead: line => codexRequester.errorOutput += line + "\n"
        }
